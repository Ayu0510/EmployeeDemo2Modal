            ...
            $('#demo').pagination({
                ...
                callback: function (data, pagination) {
                    ...
                    $.each(data, function (index, item) {
                        html = `
                        <tr class="view">
                            <td class="text-center"><button onclick="clickMinus(this)" class="btn-light parent btn-outline-light rounded-5" data-child="${item.firstName}-${item.id}" type="button" id="detailstoggle" data-toggle="modal" data-target="#employeeData"><i id="plusMinus" class="fa fa-plus"></i></td>
                         ...
                         <tr class="child ${item.firstName}-${item.id} animate__animated animate__fadeInDown animate__fast hidden">
                            <td></td>
                            <td colspan="6"><h5 class="text-white">Date Of Birth:${item.dateOfBirth}</h5>
                            <h5 class="text-white">Date Of Joining:${item.dateOfJoining}</h5>
                            <h5 class="text-white">Skills:${item.skillNames}</h5>
                            <h5 class="text-white">Details:${item.description}</h5></td>
                        </tr>
                        `;
